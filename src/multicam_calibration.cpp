...
namespace multicam_calibration {
  ...
  MulticamCalibration::MulticamCalibration(const ros::NodeHandle &nh) : nh_(nh) {
  }

  bool MulticamCalibration::initialize() {
    ...
    nh_.getParam("use_approximate_sync", use_approximate_sync_);
    ROS_INFO_STREAM((use_approximate_sync_ ? "" : "not ") <<  "using approximate sync");
    std::string filename;
    nh_.param("corners_file", filename, std::string(""));
    detector_.reset(new MultiCamApriltagDetector(nh_));
    nh_.param<int>("skip_num_frames", skipFrames_, 1);
    bool fixIntrinsics;
    nh_.param<bool>("fix_intrinsics", fixIntrinsics, false);
    calibrator_->setFixAllIntrinsics(fixIntrinsics);
    nh_.param<bool>("record_bag", record_bag_, false);
    nh_.param<std::string>("bag_base", bag_file_, std::string("~/.ros/multicam_calibration"));
    if (record_bag_) {
    ...
    for (const auto &camdata : cameras_) {
      if (cameras_.size() > 1) {
        ...
        singleCamSub_ = nh_.subscribe(camdata.rostopic, 1,
                                     &MulticamCalibration::callback1, this);
      }
      imagePub_.push_back(nh_.advertise<ImageMsg>(camdata.name + "/detected_image", 1));
      tagCountPub_.push_back(nh_.advertise<std_msgs::UInt32>(camdata.name + "/num_detected_tags", 1));
    }
    calibrationService_ = nh_.advertiseService("calibration", &MulticamCalibration::calibrate, this);
    parameterService_ = nh_.advertiseService("set_parameter", &MulticamCalibration::setParameter, this);
    subscribe();
    if (!filename.empty()) {
      if (readPointsFromFile(filename)) {
        if (!worldPoints_.empty()) {
          ...
          nh_.param<bool>("run_calib_on_init", runCalibOnInit, false);
          if (runCalibOnInit) {
  ...
  void MulticamCalibration::parseCameras() {
    int cam_index = 0; 
    while (true) {
      std::string cam = "cam" + std::to_string(cam_index);
      XmlRpc::XmlRpcValue lines;
      if (!nh_.getParam(cam, lines)) break; // no more cameras!
      CalibrationData calibData;
      calibData.name = cam;
      CameraIntrinsics &ci = calibData.intrinsics;
      if (!nh_.getParam(cam + "/camera_model",
                       ci.camera_model)) { bombout("camera_model", cam); }
      if (!nh_.getParam(cam + "/distortion_model",
                       ci.distortion_model)) { bombout("distortion_model", cam); }
      if (!nh_.getParam(cam + "/distortion_coeffs",
                       ci.distortion_coeffs)) { bombout("distortion_coeffs", cam); }
      if (!nh_.getParam(cam + "/intrinsics",  ci.intrinsics)) { bombout("intrinsics", cam); }
      if (!nh_.getParam(cam + "/resolution",  ci.resolution)) { bombout("resolution", cam); }
      if (!nh_.getParam(cam + "/rostopic",  calibData.rostopic)) { bombout("rostopic", cam); }
      calibData.T_cam_imu = get_transform(nh_, cam + "/T_cam_imu", zeros());
      calibData.T_cn_cnm1 = get_transform(nh_, cam + "/T_cn_cnm1", identity());
      
      nh_.param<bool>(cam + "/fix_intrinsics",  calibData.fixIntrinsics, false);
      nh_.param<bool>(cam + "/fix_extrinsics",  calibData.fixExtrinsics, false);
      nh_.param<bool>(cam + "/active",          calibData.active, true);
      if (cam_index == 0) {
    ...
    cameras_ready_ = true;
    T_imu_body_ = get_transform(nh_, "T_imu_body", zeros());
  }

  bool MulticamCalibration::calibrate(std_srvs::Trigger::Request& req,
    ...
    nh_.param<std::string>("output_filename", baseName, "calibration_output");
    nh_.param<std::string>("latest_link_name", linkName, "latest.yaml");
    nh_.param<std::string>("calib_dir", calibDir, "calib");
    nh_.param<std::string>("results_dir", resultsDir, ".");
  ...
  int MulticamCalibration::getCameraIndex(const std::string &cam) const {
  ...
  bool MulticamCalibration::setParameter(ParameterCmd::Request& req,
  ...
  void MulticamCalibration::testCalibration(const CalibDataVec &calib) {
  ...
  void MulticamCalibration::homographyTest(const CalibDataVec &results) const {
    if (worldPoints_.empty()) {
  ...
  void MulticamCalibration::writeCalibration(std::ostream &os, const CalibDataVec &results) {
    cv::Mat R[2], P[2], Q;
  ...
  void MulticamCalibration::updateCameras(const CalibDataVec &results) {
  ...
  void MulticamCalibration::subscribe() {
    switch (sub_.size()) {
    case 0:
      break;
    case 2:
      if (use_approximate_sync_) {
        ...
        approx_sync2_->registerCallback(&MulticamCalibration::callback2, this);
      } else {
        sync2_.reset(new ExactSynchronizer2(*(sub_[0]), *(sub_[1]), 2));
        sync2_->registerCallback(&MulticamCalibration::callback2, this);
      }
      break;
    case 3:
      if (use_approximate_sync_) {
        ...
        approx_sync3_->registerCallback(&MulticamCalibration::callback3, this);
      } else {
        sync3_.reset(new ExactSynchronizer3(*(sub_[0]), *(sub_[1]), *(sub_[2]), 2));
        sync3_->registerCallback(&MulticamCalibration::callback3, this);
      }
      break;
    case 4:
      if (use_approximate_sync_) {
        ...
        approx_sync4_->registerCallback(&MulticamCalibration::callback4, this);
      } else {
  ...
  bool MulticamCalibration::guessCameraPose(const CamWorldPoints &wp,
  ...
  bool MulticamCalibration::readPointsFromFile(const std::string &fname) {
  ...
  void MulticamCalibration::process(const std::vector<ImageConstPtr> &msg_vec) {
  ...
  void MulticamCalibration::publishDebugImages(const std::vector<ImageMsg::ConstPtr> &msg_vec,
  ...
  void MulticamCalibration::publishTagCounts() {
  ...
  void MulticamCalibration::callback1(ImageConstPtr const &img0) {
    std::vector<ImageMsg::ConstPtr> msg_vec = {img0};
    process(msg_vec);
  }

  void MulticamCalibration::callback2(ImageConstPtr const &img0, ImageConstPtr const &img1) {
    std::vector<ImageMsg::ConstPtr> msg_vec = {img0, img1};
    process(msg_vec);
  }

  void MulticamCalibration::callback3(ImageConstPtr const &img0, ImageConstPtr const &img1,
                                     ImageConstPtr const &img2) {
    std::vector<ImageMsg::ConstPtr> msg_vec = {img0, img1, img2};
    process(msg_vec);
  }
  void MulticamCalibration::callback4(ImageConstPtr const &img0, ImageConstPtr const &img1,
                                     ImageConstPtr const &img2, ImageConstPtr const &img3) {
    std::vector<ImageMsg::ConstPtr> msg_vec = {img0, img1, img2, img3};
    process(msg_vec);
  }

  void MulticamCalibration::callback5(ImageConstPtr const &img0, ImageConstPtr const &img1,
                                     ImageConstPtr const &img2, ImageConstPtr const &img3,
  ...
  void MulticamCalibration::callback6(ImageConstPtr const &img0, ImageConstPtr const &img1,
                                     ImageConstPtr const &img2, ImageConstPtr const &img3,
